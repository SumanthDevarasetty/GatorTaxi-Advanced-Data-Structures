...
public class RedBlackTree {
    ...
    public List<RedBlackNode> searchInRange(int startRideNumber, int endRideNumber) {
        ...
        searchInRangeRecursive(root, startRideNumber, endRideNumber, result);
        return result;
    }

    private void searchInRangeRecursive(RedBlackNode root, int startRideNumber, int endRideNumber, List<RedBlackNode> nodes) {
        if(root == nil) return;
        if(root.getrideNumber() > startRideNumber) {
            searchInRangeRecursive(root.getLeft(), startRideNumber, endRideNumber, nodes);
        }

        if(root.getrideNumber() >= startRideNumber && root.getrideNumber() <= endRideNumber) {
            nodes.add(root);
        } 
        
        if(root.getrideNumber() < endRideNumber) {
            searchInRangeRecursive(root.getRight(), startRideNumber, endRideNumber, nodes);
        }
    }
}